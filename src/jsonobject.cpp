...
JsonObject *JsonObject::getAsObject(std::string_view const &key) const { return getElementAsObject(m_items.at(key), key); }
JsonArray *JsonObject::getAsArray(std::string_view const &key) const { return getElementAsArray(m_items.at(key), key); }
